...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const handleCopyExecSummary = () => {
    ...
    const summaryText = [
      ...
      `• Smartsheet WBS: 30-Column Deterministic WBS ${scenario.smartsheetConfig?.sheetId ? `(Synced: ${scenario.smartsheetConfig.sheetId})` : '(Ready for Direct Trigger)'}`,
      `═══════════════════════════════════════════════════════════════════`,
      ...
      <div className="w-full max-w-7xl mx-auto px-3 sm:px-4 py-2 flex items-center justify-between gap-2">
            ...
            <div ref={scenarioDropdownRef} className="relative shrink-0">
              ...
              {dropdownOpen && (
                <div className="absolute left-0 mt-1.5 w-80 bg-white border border-slate-200 rounded-sm shadow-xl z-50 p-2 space-y-1.5 animate-in fade-in slide-in-from-top-2 duration-150 max-h-[80vh] overflow-y-auto">
                  ...
                  <div className="px-2 py-1 text-[10px] font-bold uppercase tracking-widest text-slate-400 border-t border-slate-100 pt-2">
                  ...
                  {PRESET_SCENARIOS.map((preset) => (
                    <button
                      ...
                      className={`w-full text-left p-2 rounded-sm text-xs transition cursor-pointer ${
                    ...
                    >
                      <div className="font-bold flex items-center justify-between">
                        {preset.name}
                        {scenario.id === preset.id && <CheckCircle2 size={13} className="text-white shrink-0" />}
                      </div>
                      <div className={`text-[11px] line-clamp-1 mt-0.5 ${scenario.id === preset.id ? 'text-slate-300' : 'text-slate-500'}`}>
                        {preset.description}
                      </div>
                    </button>
                  ))}
                ...
                <div className="absolute left-0 mt-1.5 w-68 bg-white border border-slate-200 rounded-sm shadow-xl z-50 p-1.5 space-y-1 animate-in fade-in slide-in-from-top-2 duration-150">
                  ...
                  {onOpenFrameworkSlider && (
                    ...
                    >
                      <Sparkles size={13} className="text-indigo-600" />
                      <div>
                        <div className="font-bold">Framework Methodology Slider</div>
                        <div className="text-[10px] text-slate-500 font-normal">One-slider overview with audio narration</div>
                      </div>
                    </button>
                  )}
                  ...
                  <button
                    ...
                    className="w-full text-left px-2.5 py-1.5 rounded-sm text-xs font-semibold text-slate-700 hover:bg-slate-100 flex items-center gap-2 transition"
                  >
                    <Layers size={13} className="text-slate-600" />
                    <div>
                      <div className="font-bold">Compare Blueprints</div>
                      <div className="text-[10px] text-slate-500 font-normal">Side-by-side scenario variance</div>
                    </div>
                  </button>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          {onOpenSmartsheetExport && (
            <button
              type="button"
              onClick={onOpenSmartsheetExport}
              className={`px-2.5 py-1.5 rounded-sm text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 transition cursor-pointer shadow-xs border shrink-0 ${
                scenario.isScheduleFrozen
                  ? 'bg-gradient-to-r from-[#002A54] via-[#004A8F] to-[#0073EA] hover:from-[#001D3D] hover:to-[#004A8F] text-white border-sky-400/50 shadow-sm'
                  : 'bg-sky-50 hover:bg-sky-100 text-sky-950 border-sky-300'
              }`}
              title="Direct Smartsheet Project Plan Trigger (WBS, Tasks, Dates, Gates)"
            >
              <Sparkles size={12} className={scenario.isScheduleFrozen ? 'text-sky-300' : 'text-[#0073EA]'} />
              <span className="font-bold">Smartsheet</span>
              <span className={`text-[8px] font-mono px-1 py-0.2 rounded-2xs font-bold uppercase ${
                scenario.isScheduleFrozen
                  ? 'bg-emerald-400 text-emerald-950'
                  : 'bg-sky-200 text-sky-950'
              }`}>
                {scenario.isScheduleFrozen ? 'READY' : 'FREEZE REQ'}
              </span>
            </button>
          )}
              ...
              <div className="absolute right-0 mt-1.5 w-60 bg-white border border-slate-200 rounded-sm shadow-xl z-50 p-1.5 space-y-1">
                {onOpenSmartsheetExport && (
                  <button
                    ...
                    className="w-full text-left px-2.5 py-1.5 rounded-sm text-xs font-bold text-sky-950 bg-sky-50 hover:bg-sky-100 flex items-center gap-2 border border-sky-200 cursor-pointer"
                  >
                    <Sparkles size={13} className="text-[#0073EA] shrink-0" />
                    <div>
                      <div>Trigger Smartsheet Plan</div>
                      <div className="text-[9px] text-sky-700 font-normal">WBS, tasks, dates & governance</div>
                    </div>
                  </button>
                )}
                {onOpenSlideDeck ? (
