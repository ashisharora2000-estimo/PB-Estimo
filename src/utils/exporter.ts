...
export function exportCommercialsToCsv(scenario: ProjectScenario, data: CalculatedProjectData) {
  ...
  const rows: string[][] = [
    ...
    ...(scenario.deliveryMix.nearshore > 0 ? [['Nearshore Delivery Share', `${scenario.deliveryMix.nearshore}%`, `${Math.round(data.hoursByRegion.nearshore)} Hours`]] : []),
    ...
    ...(scenario.deliveryMix.nearshore > 0 ? [['Nearshore', Math.round(data.hoursByRegion.nearshore).toString(), `${scenario.deliveryMix.nearshore}%`, (data.hoursByRegion.nearshore / 160).toFixed(1), (data.hoursByRegion.nearshore / (scenario.projectWeeks * 40)).toFixed(2)]] : []),
